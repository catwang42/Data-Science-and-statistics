...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    # Your code here
    loss = label*(sum(theta*feature_vector)+theta_0)
    if loss<1:
        hinge_loss = 1-loss
    else:
        hinge_loss = 0
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    final=0
    for i in range(N):
        agument = labels[i]*(sum(theta*feature_matrix[i])+theta_0)
        if agument < 1:
            hinge_loss = 1 - agument
        else:
            hinge_loss = 0
...
def perceptron_single_step_update(
    ...
    """
    # Your code here
    if label*(sum(current_theta*feature_vector)+current_theta_0)<=0:
